...
export default function AddChartDialog({ open, onClose, onAdd, editingChart }: AddChartDialogProps) {
  ...
  useEffect(() => {
    if (editingChart && open) {
      setDataSource(editingChart.dataSource);
      setDimension(editingChart.dimension);
      setSelectedMetric(editingChart.metrics[0] ?? '');
      setChartType(editingChart.chartType);
  ...
  const handleSubmit = () => {
    const config: ChartConfig = {
      id: editingChart?.id ?? `chart-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      ...
      aggregation,
      chartType,
      layout: editingChart?.layout ?? { x: 0, y: Infinity, w: 4, h: 3 },
    };
    onAdd(config);
    // Reset
    setTitle('');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <Card className="w-[480px] max-h-[90vh] overflow-auto shadow-xl">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">{editingChart ? '编辑图表' : '添加图表'}</CardTitle>
        </CardHeader>
